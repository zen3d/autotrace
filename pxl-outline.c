...
find_outline_pixels(bitmap_type bitmap, color_type *bg_color,
    ...
    for (row = 0; row < BITMAP_HEIGHT (bitmap); row++) {
        for (col = 0; col < BITMAP_WIDTH (bitmap); col++) {
            ...
            else CHECK_FATAL ();    /* FREE(DONE) outline_list */
...
at_bool is_valid_dir(unsigned short row, unsigned short col, direction_type dir, bitmap_type bitmap,
                     bitmap_type marked) {
...
next_unmarked_pixel(unsigned short *row, unsigned short *col,
    ...
    if ((*row != orig_row || *col != orig_col) &&
        (!(is_other_dir_marked(orig_row, orig_col, test_dir, *marked)
           && is_other_dir_marked(orig_row + COMPUTE_DELTA(ROW, test_dir), orig_col + COMPUTE_DELTA(COL, test_dir),
                                  test_dir, *marked))))
        return true;
    else
        return false;
