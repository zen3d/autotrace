...
typedef enum {
    NATURAL, TANGENT, PERIODIC, CYCLIC, ANTICYCLIC
} SPLINE_END_TYPE;
    ...
    for (this_list = 0; this_list < SPLINE_LIST_ARRAY_LENGTH (shape);
        ...
        for (this_spline = 0; this_spline < SPLINE_LIST_LENGTH (list);
            ...
            if (SPLINE_DEGREE (s) == LINEARTYPE) {
                ...
                pnt.xp = ROUND(startx * RESOLUTION);
                pnt.yp = ROUND(starty * RESOLUTION);
                xypnt_add_pnt(vec, pnt);
                pnt.xp = ROUND(CONTROL1(s).x * RESOLUTION);
                pnt.yp = ROUND(CONTROL1(s).y * RESOLUTION);
                xypnt_add_pnt(vec, pnt);
                pnt.xp = ROUND(CONTROL2(s).x * RESOLUTION);
                pnt.yp = ROUND(CONTROL2(s).y * RESOLUTION);
                xypnt_add_pnt(vec, pnt);
                pnt.xp = ROUND(END_POINT(s).x * RESOLUTION);
                pnt.yp = ROUND(END_POINT(s).y * RESOLUTION);
                xypnt_add_pnt(vec, pnt);
...
void output_layer(FILE *dxf_file,
    ...
    for (this_list = 0; this_list < SPLINE_LIST_ARRAY_LENGTH (shape);
         this_list++) {
        spline_list_type list = SPLINE_LIST_ARRAY_ELT (shape, this_list);
        color_type curr_color = (list.clockwise && shape.background_color != NULL) ? *(shape.background_color)
                                                                                   : list.color;
